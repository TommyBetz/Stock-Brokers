...
def main(inputs, output):
	...
	#### GET APPLE Related words 
	APPLE_Word_List = ['apple','aapl', 'airpod', 'app store', 'earpods', 'homepod', 'imac', 
	'itablet', 'magic trackpad', 'magic mouse', 'iphone', 'ipad', 'ipod', 'macbook', 'ios',
	'itunes','iwatch'] 

	joint_words = ['steve jobs', 'tim cook']

	APPLE_unwanted_list = ['fruit', 'cherries', 'berries', 'berry', 'food', 'pie', 'nutrition', 'pear', 'citrus', 'mango', 
	'grape', 'peach', 'apricot', 'mango', 'plum', 'avocado', 'ripe', 'melon', 'cherimoya', 
	'tropical', 'pineapple', 'orange', 'vegetable', 'tomato', 'fig']
	...
	def filter_appl_news(body):
		...
		for word in body.split():
			if word in APPLE_Word_List and word not in APPLE_unwanted_list:
				status = 1
			if word in APPLE_unwanted_list:
				status = 0
				break
	...
	###################################################
	# filter only apple related news
	df_headline = df_news_data.withColumn('company', filter_appl_news(df_news_data.body))
	# retain AAPL related records only
	df_frequency = df_headline.filter(df_headline.company == 'AAPL')
	# count the number of times a the apple related word is found in the body 
	df_frequency = df_frequency.withColumn('company_frequency', count_company_frequency(df_headline.body)) 
	# retain the records if at least 1 apple related word is present
