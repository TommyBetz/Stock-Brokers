# Command to run: time spark-submit Processing_Twitter_Data_version_2.py /path_to_input_folder /path_to_output_folder
import datetime
...
for path, subdirs, files in os.walk(root):
	for index, name_of_file in enumerate(files):
		...
		df_time_lang_fields_filtered.write.mode('append').parquet(output)
